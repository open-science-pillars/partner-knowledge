...
A provider repository: knowledge bundles keyed by the organization that signs their facts.

## Intended scope

Provider bundles signed by organizations outside NASA's data centers
(national agencies, international programs, research consortia), keyed
by who signs the facts, the way the PO.DAAC and ESDIS bundles are keyed
in nasa-daac-knowledge. Sphere classification answers who asks; provider
stewardship answers who signs; neither overrides the other. A bundle
names the spheres its concepts serve; the repository declares none at
repository level.

## Ownership

Owned by @open-science-pillars/provider-stewards (`CODEOWNERS`); one person
...
here relies on: None engaged yet. A partner bundle arrives with its steward, who signs its concepts; the steward team is created in build-kit's registry when the first bundle does.
